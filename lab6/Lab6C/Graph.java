...
public class Graph implements Serializable
{
   ...
   public int Kruskal() {
      ...
      // Construct a DisjointSets object that will hold sets of vertices
      ds = new DisjointSets(numVertices);

      // Add Edges into Heap
      heap = new Heap((numVertices-1)*(numVertices-1), Heap.MIN);
      ...
      // remove edge from heap and check if edge will form a cycle
      while (ds.numSets() > 1) {
         curEdge = (Edge)heap.remove();

         DisjointSets.Set setV1 = ds.find(curEdge.v1);
         DisjointSets.Set setV2 = ds.find(curEdge.v2);
    
         // if part of different sets, no cycle is formed, union them
         if (!setV1.equals(setV2)) {
            //System.out.println("(" + curEdge.v1 + "," + curEdge.v2 + ") - " + curEdge.weight);
            ds.union(setV1, setV2);
            weightTotal += curEdge.weight;
         }
      }
      return weightTotal;
   ...
   private class Edge implements Comparable{
      ...
      public int compareTo(Object o) {
         Edge x = (Edge)o;
         if (this.weight == x.weight) return 0;
