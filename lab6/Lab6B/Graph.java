...
public class Graph implements Serializable
{
   ...
   public int fewestEdgePath(int start, int stop) 
   {
      ...
      // v points to starting vertex
      // initialize num of steps to get from start to start (0 steps) 
      // v has been visited
      int v = start; 
      numSteps[v] = 0; 
      visited[v] = true;  
      ...
      while (!Q.isEmpty()) {
         ...
         for (int i = 0; i < numVertices; i++) {
             if ((adj[(int)item][i] == 1) && (visited[i] == false)) { // found an unvisited neighbor!
                visited[i] = true;
                Q.enqueue((Object)i); // insert neighbor into queue
                numSteps[i] = numSteps[(int)item] + 1; // assign num of steps to get from v to neighbor
//System.out.println("Vertex " + item + " is neighbors with vertex: " + i + " and takes " + numSteps[i] + " steps");
             }
         } 
