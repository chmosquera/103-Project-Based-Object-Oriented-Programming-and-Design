...
public class ColorTetrisDriver extends Applet {
   ...
   private class ButtonActionListener implements ActionListener {
      public void actionPerformed(ActionEvent event) {
         ...
         // left swap button was clicked
            ...
            // Can only swap to the left if colMarker is NOT the left-most
            if (colMarker > 0) {
               // initiate blocks that will be swapped
               Object block_X = null;
               Object block_Y = null;

               // Assign blocks xy to the objects that will be swapped
               // Blocks xy will stay 'null' if stack is empty
               if (stacks[colMarker].height() > 0)
                  block_X = stacks[colMarker].peek();
               if (stacks[colMarker - 1].height() > 0)
                  block_Y = stacks[colMarker - 1].peek();

               // Swap blocks xy
               // If stack contains an object, must pop object before pushing
               // Only push the block if the block is not 'null'
               if (stacks[colMarker].height() > 0)
                  stacks[colMarker].pop();
               if (block_Y != null)
                  stacks[colMarker].push(block_Y);
    
               if (stacks[colMarker-1].height() > 0)
                  stacks[colMarker-1].pop();
               if (block_X != null) 
                  stacks[colMarker - 1].push(block_X);
         ...
         // right swap button was clicked
            ...
            // Can only swap to the right if colMarker is NOT the right-most
   ...
   public void removeCombos() {
      // check for 3 or more in a row horizontally and remove them

      // initiate last two stacks involved in the combo
      Stack lastCombo_1 = null; 
      Stack lastCombo_2 = null;

      // keeps track of combo streak & if there exists a combo (at least 3)
      int combo = 1;
      boolean comboFound = false;

      for (int i = 0; i < (numCols-1); i++) {
         // Initiate and assign this stack's top and adjacent stack's top
         Object thisBlockColor = null;
         Object adjBlockColor = null;

         // check if stack is empty, if not peek at the object
         if (stacks[i].height() == 0) {}
         else thisBlockColor = stacks[i].peek();  
         if (stacks[i+1].height() == 0) {}
         else adjBlockColor = stacks[i+1].peek();

         // if stacks's tops are not 'null', check if both objects are equal
         // if objects are equal the combo streak is incremented
         if ((thisBlockColor != null) && (adjBlockColor != null)) {
            if (thisBlockColor.equals(adjBlockColor))
               combo++;
            else combo = 1; // combo is broken bc objects do not equal
         }
         else combo = 1; // combo is broken bc object is null

         // There is a combo of AT LEAST 3 if combo = 3
         if (combo == 3) {
            comboFound = true;

            // pop the previous stack's top 
            stacks[i-1].pop(); 

            // keep track of the last two objects in combo (this and next stack's top)
            lastCombo_1 = stacks[i];
            lastCombo_2 = stacks[i+1];

            // combo will be decremented back to 2 to continue checking for longer combo
            combo = 2; 
         }
      }

      // If a combo was found, pop the last two objects in the combo
      // (The rest of the objects in the combo have been popped already)
      if (comboFound) {
         lastCombo_1.pop();
         lastCombo_2.pop();
      }
         ...
         if (done == false) {
            ...
            for (int j=0; j<cols; j++) {
               if (stacks[j].height()==0) {}
                  ...
                  stackIter = stacks[j].iterator();
                  int i = 0;
                  while (i < stacks[j].height()) {
                     stackColor = (Integer) stackIter.next();
