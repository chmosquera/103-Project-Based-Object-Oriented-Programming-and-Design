...
public class Lab4B
{
   ...
   private static void medianOfThree(int[] data,int first,int last)  
   {
      ...
      if (last-first+1 >= 3)  // If there are at least 3 elements
      {
         ...
         // pivot now points to the index of three[] that holds the median
         int pivot = 1;

         // Check which index in 'data' that the median is located at
         if (three[pivot] != data[first]) {
            if (three[pivot] == data[last]) {
               Swap(data, last, first);
            }
            else if (three[pivot] == data[mid]) {
               Swap(data, mid, first);
            }
         }      
